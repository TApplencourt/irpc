...
def add_provider_call(funcdef: FuncDef,
    ...
    for e, l_compound in entity2Compound(funcdef.body, entnames).items():
        for compound in l_compound:
            ...
            prov_if = If(cond=UnaryOp(op='!', expr=ID(name=provider_bool)), iftrue=Compound(block_items=(FuncCall(name=ID(name=provider), args=None), Assignment(op='=',lvalue=ID(name=provider_bool), rvalue=Constant(type='bool', value='true')))), iffalse=None)
